...
public:
	...
	template<class T> static std::span<TextureVertex> GetDecal(const OBB& box, DynamicOctTree<T>& tree, std::vector<TextureVertex>& out);
...
template<class T> inline std::span<TextureVertex> Decal::GetDecal(const OBB& box, DynamicOctTree<T>& tree, std::vector<TextureVertex>& out)
{
	glm::vec3 halfs = box.GetScale();
	glm::vec3 center = box.GetCenter();
	// Maybe the other size too
